...
const VINHETA_TRILHA_INGEST_URL = CRIACAO_INGEST_URL.replace(/\/ingest$/, "/vinheta-trilha-ingest");
const VINHETA_TRILHA_AUDIO_BASE = CRIACAO_INGEST_URL.replace(/\/ingest$/, "/vinheta-trilha-audio");
...
export function vinhetaTrilhaIngestUrl(): string {
  return VINHETA_TRILHA_INGEST_URL;
}
...
export function signVinhetaTrilhaUpload(trilhaId: string): { token: string; exp: number } {
  return sign(trilhaId, UPLOAD_TTL_MS);
}
...
export function buildVinhetaTrilhaPreviewUrl(trilhaId: string): string | null {
  if (!SECRET) return null;
  const { token, exp } = sign(trilhaId, PREVIEW_TTL_MS);
  const sig = token.split(".")[2];
  const qs = new URLSearchParams({ exp: String(exp), token: sig });
  return `${VINHETA_TRILHA_AUDIO_BASE}/${trilhaId}?${qs.toString()}`;
}
