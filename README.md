...
**Planilha Rio (`/planilha-rio`):** novo fluxo baseado nos **clientes ativos Conta Azul** (`GET /v1/pessoas`, perfil Cliente). A lista com `tipo_perfil=Cliente` às vezes **não traz `perfis`** nos itens; o código confia então na filtragem do servidor (só ignora registros quando `perfis` vem explícito e não é cliente). Por competência (mês civil) você **regista o mês** na base e usa **Sincronizar Conta Azul** para popular/atualizar linhas: contratos ativos (`/v1/contratos`), e-mail cobrança+principal, razão social e um campo **valor** heurístico quando a API expuser dados. Ao sincronizar, o portal compara com o **mês civil anterior já guardado** e marca linhas como **entrada** / **saida** ou **—** (estável só quando existia nos dois meses). **PDVs** são listas editáveis por cliente no portal (clique no nome do cliente); **importação em massa** de PDVs virá na próxima iteração. Exportação `.xlsx` na própria página. Tabelas novas `rio_comp_*` (as `rio_planilha_*` antigas permanecem no schema para migração legada até remoção). Rode `prisma migrate deploy`.
