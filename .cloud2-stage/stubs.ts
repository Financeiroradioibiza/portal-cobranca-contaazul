...
/** Stubs — devolvem estrutura vazia (recursos ainda não migrados). get_musica é servido em getMusica.ts. */
export async function registerStubRoutes(app: FastifyInstance, prefix: string): Promise<void> {
  app.get<{ Querystring: { token?: string; agenda_atualizada?: string } }>(
    `${prefix}/agendas/`,
    async (_req, reply) => reply.send({ agendas: [] }),
  );

  app.get<{ Querystring: { token?: string } }>(
    `${prefix}/vinhetas_programadas/`,
    async (_req, reply) => reply.send({ vinhetas: [] }),
  );

  app.get<{ Querystring: { token?: string } }>(
    `${prefix}/vinhetas_agendadas/`,
    async (_req, reply) => reply.send({ vinhetas: [] }),
  );
  ...
  app.get<{ Querystring: { token?: string; musica_id?: string } }>(
    `${prefix}/save_atualizadas/`,
    async (_req, reply) => reply.send({ mensagem: 'ok' }),
  );
}
