...
export function ManualEnviosPanel() {
  ...
  useEffect(() => {
    ...
    t = setTimeout(async () => {
      ...
      const { data } = await readJsonFromResponse<{ pessoas?: PessoaHit[] }>(res);
  ...
  const refreshAllEmails = async () => {
    ...
    try {
      ...
      const { data } = await readJsonFromResponse<{ atualizados?: number; falhas?: number }>(res);
      if (data) {
        setMsg(
          data.falhas
  ...
  const onSelectHit = async (hit: PessoaHit) => {
    ...
    setMsg(txt.slice(0, 260) || "Falha ao vincular pessoa.");
  };
          ...
          <div className="max-h-[min(560px,90vh)] w-full max-w-lg overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-900">
            ...
            <div className="space-y-2 p-4">
              ...
              <div className="max-h-[min(360px,50vh)] space-y-1 overflow-y-auto pr-1">
                {hitsCa.length === 0 && buscaCa.trim().length >= 2 ? (
                  <p className="py-8 text-center text-sm text-slate-500 dark:text-slate-400">
                    Nenhum resultado (ou não conectado ao Conta Azul).
                  </p>
                ) : null}
                {hitsCa.map((h) => (
                  <button
                        ...
                        <>
                          ...
                          <button
                            ...
                            onClick={() =>
                              void (async () => {
                                const res = await fetch(
                                  `/api/manual-envios/rows/${encodeURIComponent(row.id)}/refresh-email`,
                                  {
                                    method: "POST",
                                    headers: { "Content-Type": "application/json" },
                                    credentials: "include",
                          ...
                          <button
                            ...
                            onClick={() =>
                              void (async () => {
                                await fetch(`/api/manual-envios/rows/${encodeURIComponent(row.id)}/refresh-email`, {
                                  method: "POST",
                                  headers: { "Content-Type": "application/json" },
                                  credentials: "include",
