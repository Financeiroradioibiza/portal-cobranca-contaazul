...
if [[ "$MODE" == "full" ]]; then
  ...
  for f in ingest.ts audio.ts upload-audio.ts download-process.ts vinheta.ts publicar.ts enriquecer-tags.ts apagar-musica.ts refresh-tags.ts reprocess-edicao.ts player-registry.ts publishCronogramas.ts tagEnrichmentCore.ts; do
    rsync -avz -e "ssh -o BatchMode=yes" \
      "$ROOT/.cloud2-stage/$f" \
