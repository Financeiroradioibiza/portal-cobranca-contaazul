...
for f in ingest.ts ingest-from-staging.ts audio.ts upload-audio.ts staging-audio.ts download-process.ts vinheta.ts publicar.ts enriquecer-tags.ts apagar-musica.ts refresh-tags.ts reprocess-edicao.ts reanalisar-mix-trim.ts player-registry.ts publishCronogramas.ts tagEnrichmentCore.ts ops-storage.ts cleanup-scratch.ts atualizacaoPendentePulse.ts check-musica.ts; do
